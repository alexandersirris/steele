# -*- coding: utf-8 -*-
"""
Created on Sat Dec 17 01:37:44 2022
...
# Imports the EXCEL that will be the top-level and applies your table settings. It's NOW DIFFERENT FROM the next function.
def import_main(mstr_book, mstr_sht, win_sht, chn_sht):
    dfm = pd.read_excel(mstr_book, mstr_sht, index_col=None, usecols = import_cols)
    dfw = pd.read_excel(mstr_book, win_sht, index_col=None)
    dfc = pd.read_excel(mstr_book, chn_sht, index_col=None)
    return dfm, dfw, dfc
...
def daily_update(main, daily):
    ...
    updated = updated.assign(date = pd.to_datetime(timestamp()))
...
h_df3_spi, win_list, churn_list = import_main(mstr_book, mstr_sht, win_sht, chn_sht)
...
#%%
win_list, churn_list = update_churns(win_upd, churn_upd, win_list, churn_list)
...
with pd.ExcelWriter(now+out_dest) as writer:
    ...
    n_df3_spi.to_excel(writer, sheet_name=now, index = False)
